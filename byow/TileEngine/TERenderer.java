...
public class TERenderer implements Serializable {
    ...
    public void mouseChecking(RectWorld w1) {
        int xPos = (int) StdDraw.mouseX();
        int yPos = (int) StdDraw.mouseY();
        if (xPos < w1.getWIDTH() && yPos < w1.getHEIGHT()) {
            if (w1.getWorld()[xPos][yPos] == Tileset.NEW_WALL) {
                StdDraw.text(width - 5, height - 5, "Wall");
            } else if (w1.getWorld()[xPos][yPos] == Tileset.NOTHING) {
                StdDraw.text(width - 5, height - 5, "Nothing");
            } else if (w1.getWorld()[xPos][yPos] == Tileset.NEW_FLOOR) {
                StdDraw.text(width - 5, height - 5, "Floor");
            } else if (w1.getWorld()[xPos][yPos] == w1.getEnemy().getAvatarTile()) {
                StdDraw.text(width - 5, height - 5, "Avatar");
            } else if (w1.getDoorPosition().getX() == xPos && w1.getDoorPosition().getY() == yPos) {
                StdDraw.text(width - 5, height - 5, "Door");
            } else if (w1.getKeyPosition().getX() == xPos && w1.getKeyPosition().getY() == yPos) {
                StdDraw.text(width - 5, height - 5, "Key");
            }
